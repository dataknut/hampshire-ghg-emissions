---
title: "'Wider Hampshire' GHG Emissions (v2.0)"
subtitle: "2018/19 baselines and trends over time"
author: "Ben Anderson (b.anderson@soton.ac.uk, [`@dataknut`](https://twitter.com/dataknut))"
...
myLibs <- c("data.table", "flextable", "ggplot2", 
            "leaflet", "RColorBrewer", "skimr", "sf")
 ...
 * Anderson, Ben (2023) _['Wider Hampshire' GHG Emissions (v2.0): 2018/19 baselines and trends over time]()_, University of Southampton: Southampton.

# Introduction

This report estimates greenhouse gas (GHG) emissions from the 'wider Hampshire' area using:

 * estimates of total annual _[territorial-based](https://www.ons.gov.uk/economy/environmentalaccounts/articles/netzeroandthedifferentofficialmeasuresoftheuksgreenhousegasemissions/2019-07-24#measuring-the-uks-progress-to-net-zero)_ **CO2e (selected GHGs)** emissions from 2005 onwards using the latest local authority district level data from [BEIS/DESNZ](https://www.gov.uk/government/collections/uk-local-authority-and-regional-greenhouse-gas-emissions-national-statistics). *This data now includes CO2, CH4 and N2O estimates from 2005 onwards*;
 * estimates of total annual _[territorial-based](https://www.ons.gov.uk/economy/environmentalaccounts/articles/netzeroandthedifferentofficialmeasuresoftheuksgreenhousegasemissions/2019-07-24#measuring-the-uks-progress-to-net-zero)_ **CO2e (all GHGs)** emissions for 2018/19 using the [CSE Impact tool](https://impact-tool.org.uk/download) dataset;
  * estimates of total annual _[consumption-based](https://www.ons.gov.uk/economy/environmentalaccounts/articles/netzeroandthedifferentofficialmeasuresoftheuksgreenhousegasemissions/2019-07-24#the-uks-carbon-footprint)_ **CO2e (all GHGs)** emissions for 2018/19 using the [CSE Impact tool](https://impact-tool.org.uk/download) dataset.
...
The analysis is carried out for the 'Wider Hampshire' area as defined in the map below (see Figure \@ref(fig:simpleLAMap)). This area includes _all 14_ local authorities in  Hampshire **including the unitary authorities of Portsmouth, Southampton and the Isle of Wight**. Note that this differs from the baseline estimated by The Carbon Trust for the Hampshire County Council [Climate Change Strategy](https://www.hants.gov.uk/landplanningandenvironment/environment/climatechange/whatarewedoing/climatechangestrategy) which _excluded_ the unitary authorities of Portsmouth, Southampton and the Isle of Wight. 

```{r mapBoundaries}
gis <- list()

gis$lsoa_lookup <- data.table::fread(path.expand("~/Library/CloudStorage/Dropbox/data/UK_lookups/LSOA_lookup_solent.csv.gz"))

gis$la_boundaries <- sf::read_sf(path.expand("~/Library/CloudStorage/Dropbox/data/UK_boundaries/Local_Authority_Districts__December_2018__Boundaries_GB_BFC.shp/Local_Authority_Districts__December_2018__Boundaries_GB_BFC.shp"))
gis$la_boundaries$la_name <- gis$la_boundaries$LAD18NM
gis$hampshre_la_boundaries <- getSolent(gis$la_boundaries) # defined in R/functions.R

```

```{r simpleLAMap, fig.cap = "Local authorities defining 'Wider Hampshire'"}

st_coord_sys <- sf::st_crs(gis$hampshre_la_boundaries) # check coord system
...
if(st_coord_sys$epsg != 4326){
 gis$hampshre_la_boundaries_leaflet <- st_transform(gis$hampshre_la_boundaries, "+proj=longlat +datum=WGS84")
}

leaflet::leaflet(gis$hampshre_la_boundaries_leaflet) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(fillOpacity = 0.2, 
              weight = 1.5, 
              popup = ~(paste0(LAD18NM)
                      ), # popups clicked
...
message("Number of emissions categories (all local authorities): ", uniqueN(la_ghg_DT$`LA GHG Sub-sector`))
message("Number of local authorities (all): ", uniqueN(la_ghg_DT$`Local Authority`))
t <- la_ghg_DT[, .(`N local authorities` = uniqueN(`Local Authority`)), keyby = .(Country)]

makeFlexTable(t, cap = "Number of local authorities per UK nation (unalocated = large electricity users/unallocated consumption/unkown location)")

message("Number of emissions categories (Hampshire local authorities): ", uniqueN(hampshire_la_ghg_DT$`LA GHG Sub-sector`))
message("Number of local authorities (Hampshire local authorities): ", uniqueN(hampshire_la_ghg_DT$`Local Authority`))

t <- hampshire_la_ghg_DT[, .(`N years of data` = uniqueN(`Calendar Year`),
                             `N emissions categories` = uniqueN(`LA GHG Sub-sector`)), keyby = .(`Local Authority`, `Local Authority Code`)]

makeFlexTable(t, cap = "Wider Hampshire local authorities and data availability")
...
It has been suggested that carbon sequestration through land-use and land-use change should be primarily used to offset '[fast carbon cycle](https://www.earthobservatory.nasa.gov/features/CarbonCycle/page3.php)' agricultural emissions rather than to offset (and thus enable) the continued burning of '[slow carbon cycle](https://www.earthobservatory.nasa.gov/features/CarbonCycle/page2.php)' fossil fuels.  Figure \@ref(fig:biogenicNetZero) shows how close Hampshire is to net-zero biogenic emissions under the definitions included in the plot. Note that this excludes emissions from Agricultural gas and electricity use which are assumed to be from the burning of fossil fuels and thus part of the slow carbon cycle.

```{r biogenicNetZero, fig.cap = "How close is Hampshire to biogenic Net-Zero?"}

biogenic <- hampshire_la_ghg_DT[(subSectorLabel %like% "Agric" & 
                                   !(subSectorLabel %like% "Gas") &
                                   !(subSectorLabel %like% "Elec")) |
                                  sectorLabel %like% "LULU", .(`Total kT CO2e` = sum(`Territorial emissions (kt CO2e)`, na.rm = TRUE)), keyby = .(`Calendar Year`, subSectorLabel)]


p <- ggplot2::ggplot(biogenic, aes(x = `Calendar Year`, 
                       y = `Total kT CO2e`,
                       fill = subSectorLabel)) +
  geom_col(position = "stack") +
  scale_fill_manual(values = desnz_emissions_colours) +
...
netSum <- sum(biogenic[`Calendar Year` == max(`Calendar Year`)]$`Total kT CO2e`)

absNeg <- sum(biogenic[`Calendar Year` == max(`Calendar Year`) &
                         `Total kT CO2e` < 0, `Total kT CO2e`])
absPos <- sum(biogenic[`Calendar Year` == max(`Calendar Year`) &
                         `Total kT CO2e` > 0, `Total kT CO2e`])
 
pc <- 100*(abs(absNeg)/absPos)

if(netSum > 0){
  netSumTxt <- "positive, so emissions are larger than the level of sequestration (carbon capture). In this sense Hampshire is not yet biogenic net-zero."
} else {
  netSumTxt <- "negative, so emissions are lower than the level of sequestration (carbon capture). In this sense Hampshire is biogenic net negative!"
}
```

In the most recent year, Hampshire had net biogenic emissions using the categories defined in the plot of `r round(netSum)` kT CO2e (negative emissions are `r round(pc)` % of gross emissions). This value is `r netSumTxt`

## 2019 baseline

Table \@ref(tab:hampshireDNZESCO2Total) shows the total CO2e emissions under this method for 2019 for all `r uniqueN(hampshire_la_ghg_DT$la_name)` districts. 
...
message("Total emissions for 2019: ", round(sum(hampshire_la_ghg_DT[`Calendar Year` == 2019]$`Territorial emissions (kt CO2e)`)), " kT CO2e")
...
makeOrderedBarPlot <- function(dt, year){
  ...
  res$ft <- makeFlexTable(dt[, .(Source = subSectorLabel, 
                           `Total kT CO2e`, `% of gross`)][order(-`Total kT CO2e`)], 
                cap = paste0("CO2e emissions sorted by magnitude (Hampshire, ", year , ")")
  )
