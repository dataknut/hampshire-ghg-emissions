...
These are calculated under the territorial emissions method and include _only_ CO2 emissions. Beis have produced a useful [mapping tool](https://naei.beis.gov.uk/laco2app/) which can be used to compare the spatial distribution of different emissions sources at district level.

## Latest totals for 'Hampshire'
...
p + 
  ...
  annotate("text", x = "Transport Other", y = pc_75, label = "50%", colour = "grey") +
...
p + 
  ...
  annotate("text", x = "Other Transport (t CO2e)", y = pc_75, label = "50%", colour = "grey") +
...
p + 
  ...
  annotate("text", x = "Housing - Oil (t CO2e)", y = pc_75, label = "50%", colour = "grey") +
 ...
 * CSE territorial emissions (all GHG): **`r dkUtils::tidyNum(sumCSETerr_kt,0)`** kT CO2e - `r dkUtils::tidyNum(pc_tot_diff,0)` % higher than the BEIS total
 ...
 * Power generation (`r dkUtils::tidyNum(cse_terr_powerGen,0)` T CO2e)
...
These figures draw attention to the significant emissions due to aviation which are not included in the BEIS LA level data. Indeed, `r dkUtils::tidyNum(pc_diff,0)` % of the `r dkUtils::tidyNum(pc_tot_diff,0)` % increase from the BEIS figure comprises emissions from:
  ...
  * _Power generation_ (`r dkUtils::tidyNum(cse_terr_powerGen,0)` kT CO2e)
