...
subtitle: "2018/19 snapshots and trends over time using the latest BEIS district level data"
...
# where is the data?
qmdParams$dataPath <- path.expand("~/Dropbox/data/") # change to suit your context

# latest BEIS data 
qmdParams$beisData <- "2005-21-local-authority-ghg-emissions-csv-dataset-update-060723.csv"
 ...
 * Anderson, Ben (2023) _[Hampshire 'County' GHG Emissions (v2.0): 2018/19 snapshots and trends over time using the latest BEIS district level data](), Report to the Hampshire County Council Climate Change [Expert Stakeholder Forum](https://www.hants.gov.uk/landplanningandenvironment/environment/climatechange/whoweworkwith/climatechangeforumevent)_. University of Southampton: Southampton.

# Introduction

This report estimates greenhouse gas (GHG) emissions from the Hampshire County using:

 * estimates of total annual _[territorial-based](https://www.ons.gov.uk/economy/environmentalaccounts/articles/netzeroandthedifferentofficialmeasuresoftheuksgreenhousegasemissions/2019-07-24#measuring-the-uks-progress-to-net-zero)_ **CO2** emissions for the period 2005 to 2021 using local authority district level data from [BEIS 2023 data](https://www.gov.uk/government/statistics/uk-local-authority-and-regional-greenhouse-gas-emissions-national-statistics-2005-to-2021);
* estimates of total annual _[territorial-based](https://www.ons.gov.uk/economy/environmentalaccounts/articles/netzeroandthedifferentofficialmeasuresoftheuksgreenhousegasemissions/2019-07-24#measuring-the-uks-progress-to-net-zero)_ **CH4** and **NO** emissions for the period XXXX to YYY using local authority district level data from [BEIS 2023 data](https://www.gov.uk/government/statistics/uk-local-authority-and-regional-greenhouse-gas-emissions-national-statistics-2005-to-2021);
...
# BEIS: Territorial CO2 emissions (2005 to current)

These official local authority/district level [BEIS National Statistics](https://www.gov.uk/government/statistics/uk-local-authority-and-regional-greenhouse-gas-emissions-national-statistics-2005-to-2021) use the end-user [territorial emissions method](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/996059/local-authority-co2-emissions-technical-report-2020.pdf) (_"meaning CO2 emissions that occur within the UK’s borders"_) and include _only_ CO2 emissions. As a result international aviation and shipping are _excluded_ from these estimates.

Note also that:

"_The end user basis for reporting emissions has been chosen for this dataset because it accounts for the emissions from energy use at the local level and does not penalise local areas for emissions from the production of energy which is then ‘exported’ to and used in other areas._" ([technical report](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/996059/local-authority-co2-emissions-technical-report-2020.pdf), p8)
...
# BEIS LA co2 data
beis_orig <- data.table::fread(paste0(qmdParams$dataPath,
 "beis/localAuthority/allGHG/",qmdParams$beisData))

beis_la_ghg_DT <- copy(beis_orig) # a real copy
# we have a new column: 'Greenhouse gas'
# the column names now differ accordingly
beis_la_ghg_DT[, subSectorLabel := `LA GHG Sub-sector`]
# amend category labels for clarity and ordering
# beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Agriculture",
#      "Industry: Agriculture",
#      subSectorLabel)
#      ]
beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Large Industrial Installations",
     "Industry: Large Industrial Installations",
     subSectorLabel)
]
beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Road Transport (A roads)",
     "Transport: (A roads)",
     subSectorLabel)
]
beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Road Transport (Minor roads)",
     "Transport: (Minor roads)",
     subSectorLabel)
]
beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Road Transport (Motorways)",
     "Transport: (Motorways)",
     subSectorLabel)
]
beis_la_ghg_DT[, subSectorLabel := ifelse(subSectorLabel == "Diesel Railways",
     "Transport: Diesel Railways",
     subSectorLabel)
]
beis_la_ghg_DT[, la_name := `Local Authority`]
...
beis_la_ghg_DT[, subSectorLabelFact := factor(subSectorLabel)]

catList <- unique(beis_la_ghg_DT$subSectorLabelFact) # this is not alphabetical - why?
...
hampshire_beis_la_ghg_DT <- getSolent(beis_la_ghg_DT)
# set a flag for the 11 districts in case we need it
hampshire_beis_la_ghg_DT[, widerHampshire := ifelse(la_name == "Southampton" |
                                                      la_name == "Portsmouth" |
                                                      la_name == "Isle of Wight",
                         "Wider Hampshire",
                         "Hampshire CC")]
message("Number of emissions categories: ", uniqueN(hampshire_beis_la_ghg_DT$`LA GHG Sub-sector`))
message("Number of districts: ", uniqueN(hampshire_beis_la_ghg_DT$`Local Authority`))
message("Total emissions for 2019: ", round(sum(hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]$`Territorial emissions (kt CO2e)`)), " kT CO2")
message("Total emissions deemed to be 'within the scope of influence of LAs' for 2019: ", round(sum(hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]$`CO2 emissions within the scope of influence of LAs (kt CO2e)`)), " kT CO2")

qmdParams$yearRange <- paste0(min(hampshire_beis_la_ghg_DT$`Calendar Year`), 
                              "-",
                              max(hampshire_beis_la_ghg_DT$`Calendar Year`)
                              )
```
...
```{r hampshireBEISCO2TotalTrends, fig.height = 6, fig.cap = "BEIS CO2 emissions by category (Hampshire)"}
h_beis_sum_allYears_DT <- hampshire_beis_la_ghg_DT[, .(`Total kT CO2e` = sum(`Territorial emissions (kt CO2e)`, na.rm = TRUE)), keyby = .(`Calendar Year`, subSectorLabelFact)]
...
ggplot2::ggsave(here::here("plots", "beis", 
                           paste0(qmdParams$yearRange, "_trendsCol.png")
                           ), plot = p)
p
...
ggplot2::ggsave(here::here("plots", "beis", 
                           paste0(qmdParams$yearRange, "_trendsLine.png")
                           ), plot = p)
p
```
...
```{r hampshireBEISCO2TotalTrendsLinePlotly, fig.height = 6, fig.cap = "BEIS CO2 emissions by category (Hampshire)"}
library(plotly)
plotly::ggplotly(p)
```
...
Table \@ref(tab:hampshireBEISCO2Total) shows the total CO2 emissions under this method for 2019 for all `r uniqueN(hampshire_beis_la_ghg_DT$la_name)` districts. 

```{r hampshireBEISCO2Total}
t <- hampshire_beis_la_ghg_DT[`Calendar Year` == 2019, .(`Total kT CO2e` = sum(`Territorial emissions (kt CO2e)`, na.rm = TRUE))]
makeFlexTable(t, cap = "BEIS CO2e emissions (kT, 2019)")

sumBEIS_kt <- sum(hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]$`Territorial emissions (kt CO2e)`)

sum_within_infl <- sum(hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]$`Emissions within the scope of influence of LAs (kt CO2)`)
```
...
```{r hampshireBEISCO2TotalByCat, fig.cap = "BEIS CO2e emissions by category (Hampshire, 2019 ordered by emissions value)"}


makeOrderedBarPlot <- function(dt, year){
  
  plotDT <- dt[`Calendar Year` == year, .(`Total kT CO2e` = sum(`Territorial emissions (kt CO2e)`, na.rm = TRUE)), keyby = .(subSectorLabelFact)]

absTotal <- sum(dt[`Calendar Year` == year, abs(`Territorial emissions (kt CO2e)`)])
                
plotDT[, `% of gross` := 100*(`Total kT CO2e`/absTotal)]

makeFlexTable(plotDT[, .(Source = subSectorLabelFact, 
                                             `Total kT CO2e`, `% of gross`)], 
              cap = paste0("BEIS CO2 emissions sorted by category (Hampshire, ", year , ")")
              )

  p <- ggplot2::ggplot(plotDT, 
                       aes(x = reorder(subSectorLabelFact, -`Total kT CO2e`), 
                           y = `Total kT CO2e`,
                           fill = subSectorLabelFact)) +
    geom_col() +
    scale_fill_manual(values = beis_emissions_colours) +
...
p <- makeOrderedBarPlot(hampshire_beis_la_ghg_DT, year = 2019)
p
...
#   scale_fill_manual(values = beis_emissions_colours) #
# p
latestYear <- max(hampshire_beis_la_ghg_DT$`Calendar Year`)
```
...
```{r beisCumulativeDot, fig.cap="Plot of cumulative emissions (BEIS, 2019)"}
...
makeOrderedDotPlot <- function(dt, year){
  ...
  plotCap <- paste0(year,"\nVertical lines % of net emissions")
  # x = reorder(subSectorLabelFact, -`Total kT CO2e`)
  p <- ggplot2::ggplot(t, aes(x = reorder(subSectorLabelFact, -`Total kT CO2e`), 
                              y = cumulative,
                              colour = subSectorLabelFact)) +
    geom_point() +
    ylim(0,NA) +
...
p <- makeOrderedDotPlot(hampshire_beis_la_ghg_DT, year = 2019)
...
```{r discBEISCO2e}
h_beis2019 <- hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]
h_beis_allYears <- hampshire_beis_la_ghg_DT[, .(kT_co2_beis = sum(`Territorial emissions (kt CO2e)`)), keyby = .(`Calendar Year`, `LA GHG Sub-sector`)]

beis_trans <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Motorway" | `LA GHG Sub-sector` %like% "roads", `Territorial emissions (kt CO2e)`])

beis_motorways <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Motorway", `Territorial emissions (kt CO2e)`])

beis_d_gas <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Domestic Gas", `Territorial emissions (kt CO2e)`])
beis_d_elec <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Domestic Electricity", `Territorial emissions (kt CO2e)`])

beis_forest <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Forest", `Territorial emissions (kt CO2e)`])

beis_grassland <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Grass", `Territorial emissions (kt CO2e)`])

beis_wetlands <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Wetland", `Territorial emissions (kt CO2e)`])

beis_crop <- sum(h_beis2019[`LA GHG Sub-sector` %like% "Crop", `Territorial emissions (kt CO2e)`])

h_beis2019[, zeroC := ifelse(`LA GHG Sub-sector` %like% "Gas",
                             0,
                             `Territorial emissions (kt CO2e)`)]

h_beis2019[, zeroC := ifelse(`LA GHG Sub-sector` %like% "Electricity",
                             0,
                             `Territorial emissions (kt CO2e)`)]

h_beis2019[, zeroC := ifelse(`LA GHG Sub-sector` %like% "roads",
                             0,
                             `Territorial emissions (kt CO2e)`)]

beis_allSum <- sum(h_beis2019$`Territorial emissions (kt CO2e)`)
...
As Figure \@ref(fig:beisCumulative) showed, these levels of sequestration currently provide a negligible offset to the overall emissions. Note also that cropland was a net emitter at `r dkUtils::tidyNum(beis_crop,0)` kT. 
...
```{r colPltLatestYear}
p <- makeOrderedBarPlot(hampshire_beis_la_ghg_DT, year = max(hampshire_beis_la_ghg_DT$`Calendar Year`))
p

fname <- paste0("beis-", max(hampshire_beis_la_ghg_DT$`Calendar Year`),
                "_col.png")
ggplot2::ggsave(here::here("plots", fname), plot = p)
```

```{r dotPltLatestYear}
p <- makeOrderedDotPlot(hampshire_beis_la_ghg_DT, year = max(hampshire_beis_la_ghg_DT$`Calendar Year`))
p
...
cse_territorial_abs_DT <- data.table::fread(paste0(qmdParams$dataPath,
...
beis_terr_total <- sum(hampshire_beis_la_ghg_DT[`Calendar Year` == 2019]$`Total kT CO2e`)
...
hampshire_beis_terr_Totals <- hampshire_beis_la_ghg_DT[`Calendar Year` == 2019,
                                                       .("Total kT CO2e" = sum(`Territorial emissions (kt CO2e)`)),
                                                       keyby = .(subSectorLabelFact)]

beis_terr_Transport <- sum(hampshire_beis_terr_Totals[subSectorLabelFact %like% "road" |
                                                        subSectorLabelFact %like% "Motorway"]$`Total kT CO2e`)

cse_terr_DomGas <- hampshire_cse_terr_Totals[variable %like% "Housing - Mains gas"]$`Total kT CO2e`
beis_terr_DomGas <- hampshire_beis_terr_Totals[subSectorLabelFact %like% "Domestic Gas"]$`Total kT CO2e`
...
cse_consumption_abs_DT <- data.table::fread(paste0(qmdParams$dataPath,
...
districts_beis_2019 <- hampshire_beis_la_ghg_DT[`Calendar Year` == 2019, .(`BEIS territorial emissions (kt CO2, 2019)` = sum(`Territorial emissions (kt CO2e)`),
...
sumBEIS_hcc_kt <- sum(hampshire_beis_la_ghg_DT[widerHampshire != "Wider Hampshire" & `Calendar Year` == 2019, `Territorial emissions (kt CO2e)`])
```

Overall, the total GHG emissions for Hampshire in 2018/19 under different methodologies were found to be:

 * Carbon Trust analysis (`r uniqueN(hampshire_beis_la_ghg_DT[widerHampshire != "Wider Hampshire", la_name])` districts _excluding_ Portsmouth, Southampton and the Isle of Wight): **`r dkUtils::tidyNum(ct_total,0)`** kT CO2
 * BEIS territorial CO2e: 
   * **`r dkUtils::tidyNum(sumBEIS_hcc_kt,0)`** kT CO2e (`r uniqueN(hampshire_beis_la_ghg_DT[widerHampshire != "Wider Hampshire", la_name])` districts _excluding_ Portsmouth, Southampton and the Isle of Wight),
   * **`r dkUtils::tidyNum(sumBEIS_kt,0)`** kT CO2e (`r uniqueN(hampshire_beis_la_ghg_DT[, la_name])` districts _including_ Portsmouth, Southampton and the Isle of Wight)
 * CSE territorial emissions (all GHG, all `r uniqueN(hampshire_beis_la_ghg_DT[, la_name])` districts): **`r dkUtils::tidyNum(sumCSETerr_kt,0)`** kT CO2e - `r dkUtils::tidyNum(pc_tot_diff,0)` % higher than the BEIS total
 * CSE consumption emissions (all GHG, all `r uniqueN(hampshire_beis_la_ghg_DT[, la_name])` districts): **`r dkUtils::tidyNum(sumCSECons_kt,0)`** kT CO2e
...
Table \@ref(tab:tableCompareTerr) compares the Carbon Trust, BEIS and CSE data for 2018/19 to the extent that it is possible to do so from the data reported here.
...
hampshire_beis_Compare <- hampshire_beis_terr_Totals[, .(ktco2_beis = sum(`Total kT CO2e`)), keyby = .(compareLab)]
 ...
 * use _all_ territorial emissions (e.g. due to aviation, shipping, F-gases) and _all GHGs_ (i.e. [carbon dioxide, methane, nitrous oxide and fluorinated gases](https://naei.beis.gov.uk/overview/ghg-overview)) in the annual [BEIS district level emissions data](https://www.gov.uk/government/collections/uk-local-authority-and-regional-carbon-dioxide-emissions-national-statistics) as the benchmark for: 
 ...
 * use the BEIS district level and (aggregated to) county level data to establish [Science Based Targets](https://sciencebasedtargetsnetwork.org/take-action-now/take-action-as-a-city/) for emissions reduction that align with or improve upon national 'net-zero' targets;
